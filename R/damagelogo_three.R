...
plot_pie <- function(breaks_theta_vec){
  ...
  names = rownames(breaks_theta_vec)
  bases <- c(as.character(sapply(names, function(x) return(strsplit(x, "[_]")[[1]][1]))))
  shuffle <- c(match(c("A-left", "G-left", "C-left", "T-left"), bases[1:4]), 4+ match(c("A-right", "G-right", "C-right", "T-right"), bases[5:8]))
  strand <- c(as.character(sapply(names, function(x) return(strsplit(x, "[-]")[[1]][2]))))
  strand <- plyr::revalue(factor(strand), c("left"="5' strand break", "right"="3' strand break"))
  sum1 = rep(tapply(breaks_theta_vec[,1], strand, sum), each=4)
  breaks_theta_vec_2 <- breaks_theta_vec[,1]/sum1
  df <- data.frame("value" = substring(bases[shuffle],1,1), "category"=strand[shuffle], "percentage" = breaks_theta_vec_2[shuffle])
