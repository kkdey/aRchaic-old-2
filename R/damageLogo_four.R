...
damageLogo_four <- function(theta_pool,
  ...
  signature_left <- substring(signature_set, 8+2*flanking_bases,  8+2*flanking_bases)
  signature_right <- substring(signature_set, 10+2*flanking_bases,  10+2*flanking_bases)

  theta_left <- dplyr::tbl_df(data.frame(theta_pool)) %>% dplyr::mutate(sig = signature_left) %>% dplyr::group_by(sig) %>% dplyr::summarise_each(funs(sum)) %>% as.data.frame()
  theta_right <- dplyr::tbl_df(data.frame(theta_pool)) %>% dplyr::mutate(sig = signature_right) %>% dplyr::group_by(sig) %>% dplyr::summarise_each(funs(sum)) %>% as.data.frame()
  rownames(theta_left) <- theta_left[,1]
  rownames(theta_right) <- theta_right[,1]
  theta_left <- theta_left[,-1]
  theta_right <- theta_right[,-1]

  theta_left <- theta_left[match(c("A", "C", "G", "T"), rownames(theta_left)),]
  theta_right <- theta_right[match(c("A", "C", "G", "T"), rownames(theta_right)),]
  rownames(theta_left) <- paste0(rownames(theta_left), "-left")
  rownames(theta_right) <- paste0(rownames(theta_right), "-right")

  breaks_theta <- rbind(theta_left, theta_right)
  ...
  for(l in 1:length(prop_patterns_list)){
    damageLogo.pos.str.skeleton(pwm = prop_patterns_list[[l]],
                                ...
                                stackbarport_x = stackbarport_x,
                                stackbarport_y = stackbarport_y,
                                stackbarport_width=stackbarport_width,
                                stackbarport_height=stackbarport_height,
                                barport_x = barport_x,
                                barport_y = barport_y,
...
damageLogo.pos.str.skeleton <- function(pwm,
                                        ...
                                        stackbarport_x = 1,
                                        stackbarport_y = 0.50,
                                        stackbarport_width=0.40,
                                        stackbarport_height=0.40,
                                        barport_x = 0.5,
                                        barport_y=0.80,
                                        barport_width=0.25,
                                        barport_height=0.25){
  ...
  # vp3 <- viewport(x=stackbarport_x, y=stackbarport_y, width=stackbarport_width, height=stackbarport_height, just=c("right","bottom"))
  # pushViewport(vp3)
  # par(plt = gridPLT(), new=TRUE)
  vp3 <- viewport(width = stackbarport_width, height = stackbarport_height, x = stackbarport_x,
                  y = unit(stackbarport_y, "lines"), just = c("right","bottom"))
  p = plot_stackbar(breaks_theta_vec = breaks_theta_vec)
  print(p, vp = vp3)
...
plot_stackbar <- function(breaks_theta_vec){
